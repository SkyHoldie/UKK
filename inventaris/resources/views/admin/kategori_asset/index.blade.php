...
@section('content')
    <div class="container py-5">
        ...
        <a href="{{ route('admin.kategori_asset.create') }}" class="btn btn-gradient mb-3 d-block mx-auto">Tambah Kategori Asset</a>
                ...
                <tbody>
                    @foreach ($kategori_assets as $kategori)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('admin.kategori_asset.edit', $kategori->id_kategori_asset) }}" class="btn btn-warning btn-sm rounded-pill">Edit</a>
                                
                                <!-- Form Hapus -->
                                <form action="{{ route('admin.kategori_asset.destroy', $kategori->id_kategori_asset) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-sm rounded-pill" onclick="return confirm('Apakah Anda yakin ingin menghapus kategori ini?')">Hapus</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
